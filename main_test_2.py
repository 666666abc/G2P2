...
tit_dict = json.load(open('./data/{}_text.json'.format(data_name)))
new_dict = {}
for i in range(len(tit_dict)):
    new_dict[i] = tit_dict[str(i)]

id_lab_dict = json.load(open('./data/{}_id_labels.json'.format(data_name)))
...
edge_index = np.load('./data/{}_edge.npy'.format(data_name))

arr_edge_index = edge_index

edge_index = torch.from_numpy(edge_index).to(device)

node_f = np.load('./data/{}_f_m.npy'.format(data_name))
...
def main(args):
    ...
    clip_model.load_state_dict(torch.load('./res/{}/node_ttgt_8&12_10.pkl'.format(data_name), map_location=device))
